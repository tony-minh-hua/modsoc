...
to play-game
  ...
  ask turtles [
    set payoff 0
    let neighbors-C count (link-neighbors) with [strategy = 1]
    let neighbors-D count (link-neighbors) with [strategy = 0]
    let neighbors-T count (link-neighbors) with [strategy = 2]
...
to evolve
  ...
  ask turtles[
    ;;check to see if any neighbors exist
    if any? link-neighbors [
      let best-neighbor max-one-of (link-neighbors) [payoff] ;;who's my neighbor with the highest payoff
...
false
false
"" "set-plot-x-range (min [count link-neighbors] of turtles) (max [count link-neighbors] of turtles + 1) "
PENS
...
"pen-2" 1.0 0 -13840069 true "" "plot count turtles with [strategy = 2]"

@#$#@#$#@
## Model Information and Materials

This model is original material created by Paul E. Smaldino, building on code by Wilensky and Rand (2008).
...
* Smaldino PE (2023). Random Network. Modeling Social Behavior.  https://github.com/psmaldino/modsoc/
